...
export function PageHeader({ eyebrow, title, description, actions, titleAction, id }: PageHeaderProps) {
  const { t } = useLanguage();
    ...
    <header className="flex flex-col gap-4 pt-1 sm:flex-row sm:items-end sm:justify-between sm:gap-5 sm:pt-0">
      <div className="max-w-2xl">
        <div className="flex flex-wrap items-center gap-2 sm:mb-2">
          <p className="inline-flex min-h-7 items-center rounded-full border border-emerald-100 bg-white/75 px-3 text-[10px] font-bold uppercase tracking-[0.12em] text-emerald-700 shadow-[var(--shadow-control)] backdrop-blur sm:min-h-0 sm:border-0 sm:bg-transparent sm:px-0 sm:text-xs sm:shadow-none">
