      ...
      <main id="main-content" tabIndex={-1} className="app-page-content space-y-5 outline-none sm:space-y-6">
        <div className="space-y-4">
          <PageHeader
            ...
            title="Trading"
            description="Rekam rencana, risiko, hasil, dan refleksi. Review AI tetap advisory dan tidak pernah mengubah jurnal."
          />

          <PortfolioTabs />

          {activeTab === "journal" && (
            <div role="group" aria-label="Aksi trading" className="flex w-full flex-col gap-2.5 sm:flex-row sm:items-center">
              ...
              <Button onClick={openTradeForm} disabled={!accounts.length} className="w-full sm:w-auto">
                <Plus className="h-4 w-4" /> Catat trade
              </Button>
            </div>
          )}
        </div>
...
function Dialog({ title, eyebrow, description, saving, onClose, initialFocusRef, wide = false, children }: { title: string; eyebrow: string; description: string; saving: boolean; onClose: () => void; initialFocusRef?: React.RefObject<HTMLElement | null>; wide?: boolean; children: React.ReactNode }) { return <DialogFrame titleId="trading-dialog-title" descriptionId="trading-dialog-description" initialFocusRef={initialFocusRef} onClose={onClose} closeDisabled={saving} contentClassName={wide ? "sm:max-w-2xl" : undefined}><section><div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-slate-100 bg-white/95 px-5 py-4 backdrop-blur sm:px-6"><div><p className="text-xs font-bold uppercase tracking-[0.1em] text-emerald-700">{eyebrow}</p><h2 id="trading-dialog-title" className="mt-1 text-xl font-bold tracking-tight">{title}</h2><p id="trading-dialog-description" className="mt-1 text-xs leading-5 text-slate-500">{description}</p></div><Button variant="ghost" size="icon" onClick={onClose} disabled={saving} aria-label="Tutup form trading"><X className="h-5 w-5" /></Button></div>{children}</section></DialogFrame>; }
