      ...
      <main id="main-content" tabIndex={-1} className="app-page-content space-y-5 outline-none sm:space-y-6">
        <div className="space-y-4">
          ...
          {activeTab === "journal" && (
            <div role="group" aria-label={t("Aksi trading")} className="flex w-full flex-col gap-2.5 sm:flex-row sm:items-center">
              <Button variant="secondary" onClick={openSnapshotForm} disabled={!accounts.length} className="w-full sm:w-auto">
                <Camera className="h-4 w-4" /> {t("Update equity")}
              </Button>
              <Button onClick={openTradeForm} disabled={!accounts.length} className="w-full sm:w-auto">
        ...
        {activeTab === "reviews" ? <div id="trading-panel-reviews" role="tabpanel" aria-labelledby="trading-tab-reviews"><TradingInsights /></div> : (
          <div id="trading-panel-journal" role="tabpanel" aria-labelledby="trading-tab-journal" className="space-y-6">
            {!loading && accounts.length === 0 && <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">{t("Tambahkan akun bertipe Trading terlebih dahulu di ")}<Link href="/accounts" className="font-bold underline underline-offset-2">{t("Akun & saldo")}</Link>.</div>}
            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label={t("Ringkasan trading")}><TradeMetric label={t("Trade terbuka")} value={String(metrics.open)} hint={t("Posisi yang belum ditutup")} icon={Target} /><TradeMetric label={t("Win rate")} value={`${metrics.winRate.toFixed(0)}%`} hint={t("Dari {count} trade tertutup", { count: metrics.closed })} icon={TrendingUp} /><TradeMetric label={t("P/L tertutup")} value={`${metrics.pnl >= 0 ? "+" : "−"}$${Math.abs(metrics.pnl).toLocaleString("en-US")}`} hint={t("Sesuai mata uang akun")} icon={metrics.pnl >= 0 ? TrendingUp : TrendingDown} tone={metrics.pnl >= 0 ? "text-emerald-700" : "text-rose-700"} /><TradeMetric label={t("R rata-rata")} value={metrics.averageR === null ? t("Belum ada") : `${metrics.averageR.toFixed(2)}R`} hint={t("Hasil dibanding risiko awal")} icon={BrainCircuit} /></section>
            ...
            <Surface className="overflow-hidden">
              ...
              {loading ? <TradeSkeleton /> : filteredTrades.length === 0 ? <EmptyState icon={trades.length ? Search : Target} title={trades.length ? t("Trade tidak ditemukan") : t("Journal masih kosong")} description={trades.length ? t("Coba filter status atau kata kunci lain.") : t("Catat trade pertama lengkap dengan risiko dan alasan entry.")} action={!trades.length && accounts.length ? <Button onClick={openTradeForm}><Plus className="h-4 w-4" /> {t("Catat trade")}</Button> : undefined} /> : <div className="divide-y divide-slate-100">{filteredTrades.map((trade) => <TradeRow key={trade.id} trade={trade} accountName={accountNames.get(trade.account_id)} requesting={requestingReviewId === trade.id} onReview={() => void requestAiReview(trade.id)} dateLocale={dateLocale} />)}</div>}
            </Surface>
          </div>
        )}
      </main>
...
function TradeRow({ trade, accountName, requesting, onReview, dateLocale }: { trade: Trade; accountName?: string; requesting: boolean; onReview: () => void; dateLocale: typeof idLocale }) {
  const { t } = useLanguage();
  ...
  return (
    <article className="p-4 sm:p-5">
      ...
      <div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button variant="secondary" size="compact" onClick={onReview} disabled={requesting}>
          {requesting ? <><Loader2 className="h-3.5 w-3.5 animate-spin" /> {t("Meminta...")}</> : <><BrainCircuit className="h-3.5 w-3.5" /> {t("Minta review")}</>}
        </Button>
      </div>
    </article>
  );
...
function TradeDialog({ accounts, form, setForm, saving, error, symbolRef, onClose, onSubmit }: { accounts: Account[]; form: ReturnType<typeof createTradeForm>; setForm: React.Dispatch<React.SetStateAction<ReturnType<typeof createTradeForm>>>; saving: boolean; error: string | null; symbolRef: React.RefObject<HTMLInputElement | null>; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => Promise<void> }) {
  const { t } = useLanguage();
  return (
    <Dialog title={t("Catat trade")} eyebrow={t("Trade plan & review")} description={t("Simpan rencana sebelum hasil agar evaluasi tetap jujur.")} saving={saving} onClose={onClose} initialFocusRef={symbolRef} wide>
      <form onSubmit={onSubmit}>
        ...
        <DialogActions saving={saving} onClose={onClose} label={t("Simpan trade")} />
      </form>
    </Dialog>
  );
}
...
function SnapshotDialog({ accounts, form, setForm, saving, error, onClose, onSubmit }: { accounts: Account[]; form: ReturnType<typeof createSnapshotForm>; setForm: React.Dispatch<React.SetStateAction<ReturnType<typeof createSnapshotForm>>>; saving: boolean; error: string | null; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => Promise<void> }) {
  const { t } = useLanguage();
  return (
    <Dialog title={t("Update equity")} eyebrow={t("Trading snapshot")} description={t("Masukkan total equity termasuk saldo dan floating P/L.")} saving={saving} onClose={onClose}>
      <form onSubmit={onSubmit}>
