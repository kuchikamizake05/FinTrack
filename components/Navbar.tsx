...
export default function Navbar() {
  ...
  return <>
    ...
    <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-neutral-900 bg-[#07070f] px-4 md:hidden"><Link href="/dashboard" className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-tr from-violet-600 to-indigo-600"><TrendingUp className="h-4 w-4 text-white" /></div><span className="font-bold text-white">FinTrack</span></Link><button onClick={() => setProfileOpen((value) => !value)} aria-label="Buka profil" className="flex h-8 w-8 items-center justify-center rounded-full border border-neutral-700 bg-neutral-900 text-violet-300"><User className="h-4 w-4" /></button>{profileOpen && <ProfileMenu onClose={() => setProfileOpen(false)} onLogout={handleLogout} mobile />}</header>

    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-neutral-800 bg-[#07070f] pb-safe md:hidden"><div className="mx-auto flex h-16 max-w-lg items-center justify-around">{primaryNavigation.map((item) => { const Icon = item.icon; return <Link key={item.href} href={item.href} className={`flex h-full w-1/4 flex-col items-center justify-center gap-1 ${isActive(item.href) ? "font-bold text-violet-400" : "text-neutral-500"}`}><div className={`rounded-xl p-1.5 ${isActive(item.href) ? "border border-violet-500/20 bg-violet-600/10" : ""}`}><Icon className="h-5 w-5" /></div><span className="text-[10px] font-semibold tracking-wide">{item.name}</span></Link>; })}</div></nav>
  </>;
}
