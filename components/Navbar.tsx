...
export default function Navbar() {
  ...
  return <>
    <header className="sticky top-0 z-40 hidden w-full border-b border-neutral-900 bg-[#07070f] md:block">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <div className="flex items-center gap-8">
          <Link href="/dashboard" className="flex items-center gap-2 group"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-tr from-emerald-600 to-green-500 shadow-md shadow-emerald-500/25"><TrendingUp className="h-5 w-5 text-white" /></div><span className="text-xl font-bold tracking-tight text-[#1b2740] transition-colors group-hover:text-emerald-700">FinTrack</span></Link>
    ...
    <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-neutral-900 bg-[#07070f] px-4 md:hidden"><Link href="/dashboard" className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-tr from-emerald-600 to-green-500"><TrendingUp className="h-4 w-4 text-white" /></div><span className="font-bold text-[#1b2740]">FinTrack</span></Link><button onClick={() => setProfileOpen((value) => !value)} aria-label="Buka profil" className="flex h-8 w-8 items-center justify-center rounded-full border border-emerald-200 bg-white text-emerald-700 transition hover:bg-emerald-50"><User className="h-4 w-4" /></button>{profileOpen && <ProfileMenu onClose={() => setProfileOpen(false)} onLogout={handleLogout} mobile />}</header>

    <nav aria-label="Navigasi utama" className="fixed inset-x-7 bottom-4 z-40 rounded-[24px] border border-emerald-100 bg-white p-1 shadow-[0_10px_24px_rgba(22,101,52,0.14)] md:hidden"><div className="mx-auto flex h-12 max-w-sm items-center justify-around gap-1">{primaryNavigation.map((item) => { const Icon = item.icon; const active = isActive(item.href); return <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`flex h-full min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-[19px] transition-[background-color,color,transform] duration-200 ease-out active:scale-95 ${active ? "bg-emerald-50 font-bold text-emerald-700" : "text-slate-500 hover:bg-emerald-50"}`}><Icon className={`h-[18px] w-[18px] transition-transform duration-200 ${active ? "scale-105 stroke-[2.5]" : "stroke-2"}`} /><span className="truncate px-1 text-[9px] font-semibold tracking-tight">{item.name}</span></Link>; })}</div></nav>
  </>;
}
