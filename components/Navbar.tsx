...
export default function Navbar() {
  ...
  return <>
    ...
    <nav aria-label="Navigasi utama" className="fixed inset-x-7 bottom-2 z-40 rounded-[24px] border border-slate-200 bg-white p-1 shadow-[0_10px_24px_rgba(29,48,86,0.16)] md:hidden"><div className="mx-auto flex h-12 max-w-sm items-center justify-around gap-1">{primaryNavigation.map((item) => { const Icon = item.icon; const active = isActive(item.href); return <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`flex h-full min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-[19px] transition-[background-color,color,transform] duration-200 ease-out active:scale-95 ${active ? "bg-[#e9eeff] font-bold text-[#3558d3]" : "text-slate-500 hover:bg-slate-50"}`}><Icon className={`h-[18px] w-[18px] transition-transform duration-200 ${active ? "scale-105 stroke-[2.5]" : "stroke-2"}`} /><span className="truncate px-1 text-[9px] font-semibold tracking-tight">{item.name}</span></Link>; })}</div></nav>
  </>;
}
