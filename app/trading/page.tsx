...
import { BrainCircuit, Camera, Loader2, Plus, Target, TrendingDown, TrendingUp, X } from "lucide-react";
...
export default function TradingPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!user) return;
      const [{ data: accountData, error: accountError }, { data: tradeData, error: tradeError }, { data: snapshotData, error: snapshotError }] = await Promise.all([
      ...
      setAccounts((accountData ?? []) as Account[]); setTrades((tradeData ?? []) as Trade[]); setSnapshots((snapshotData ?? []) as Snapshot[]);
    } catch (loadError) {
  ...
  async function saveSnapshot(event: React.FormEvent) {
    ...
    if (!snapshotForm.accountId || !Number.isFinite(equity) || equity < 0) { setError("Pilih akun dan masukkan equity yang valid."); return; }
    setSaving(true); setError(null);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error("Sesi login tidak ditemukan.");
      const account = accounts.find((item) => item.id === snapshotForm.accountId);
      const { error: insertError } = await supabase.from("account_equity_snapshots").insert({ user_id: user.id, account_id: snapshotForm.accountId, equity, currency: account?.currency ?? "USD", recorded_at: new Date(snapshotForm.recordedAt).toISOString(), note: snapshotForm.note.trim() || null });
      if (insertError) throw insertError;
      setSnapshotModalOpen(false); setSnapshotForm(emptySnapshot); await loadData();
    } catch (saveError) { setError(saveError instanceof Error ? saveError.message : "Gagal menyimpan snapshot equity."); } finally { setSaving(false); }
  }
  ...
  async function requestAiReview(tradeId: string) {
    ...
    try {
      ...
      alert("Review AI sedang diproses. Buka tab AI Insights di halaman Trading beberapa saat lagi untuk melihat hasilnya.");
  ...
  return <div className="min-h-screen bg-[#050507] pb-24 text-[#f7f8f8] md:pb-6"><Navbar /><main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6">
    <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-violet-500">Trading journal</p><h1 className="mt-1 text-2xl font-bold text-[#17233b]">Forex Journal</h1><p className="mt-1 text-sm text-slate-500">Rekam setup, risiko, emosi, dan hasil trade HFM atau Exness. AI hanya memberi review, bukan mengubah data Anda.</p></div>{activeTab === "journal" && <div className="flex gap-2"><button onClick={() => setSnapshotModalOpen(true)} disabled={accounts.length === 0} className="inline-flex items-center justify-center gap-2 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2 text-xs font-bold text-violet-700 disabled:opacity-50"><Camera className="h-4 w-4" /> Update equity</button><button onClick={() => setModalOpen(true)} disabled={accounts.length === 0} className="inline-flex items-center justify-center gap-2 rounded-xl bg-violet-600 px-3 py-2 text-xs font-bold text-white hover:bg-violet-500 disabled:cursor-not-allowed disabled:opacity-50"><Plus className="h-4 w-4" /> Catat trade</button></div>}</section>
    <div className="inline-flex rounded-lg border border-neutral-800 bg-[#0b0b0f] p-1"><button onClick={() => setActiveTab("journal")} className={`rounded-md px-3 py-1.5 text-xs font-bold ${activeTab === "journal" ? "bg-violet-600 text-white" : "text-neutral-400 hover:text-white"}`}>Jurnal & Statistik</button><button onClick={() => setActiveTab("insights")} className={`rounded-md px-3 py-1.5 text-xs font-bold ${activeTab === "insights" ? "bg-violet-600 text-white" : "text-neutral-400 hover:text-white"}`}>AI Insights</button></div>
    {activeTab === "journal" && <div className="space-y-6">
    {error && <div className="rounded border border-rose-500/30 bg-rose-500/10 px-4 py-3 text-sm text-rose-300">{error}</div>}
    {accounts.length === 0 && !loading && <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Tambahkan akun bertipe Trading (misalnya HFM atau Exness) terlebih dahulu di <Link className="font-bold underline" href="/accounts">Akun</Link>.</div>}
    ...
    {activeTab === "insights" && <TradingInsights />}
    {snapshotModalOpen && <div className="fixed inset-0 z-50 flex items-end bg-slate-950/50 p-4 sm:items-center sm:justify-center"><form onSubmit={saveSnapshot} className="linear-panel w-full max-w-lg space-y-3 rounded-2xl p-5"><div className="flex items-center justify-between"><h2 className="font-bold text-[#17233b]">Update equity trading</h2><button type="button" onClick={() => setSnapshotModalOpen(false)} aria-label="Tutup"><X className="h-5 w-5" /></button></div><p className="text-xs text-slate-500">Masukkan total nilai akun sekarang, termasuk floating P/L dan saldo.</p><select required value={snapshotForm.accountId} onChange={(event) => setSnapshotForm({ ...snapshotForm, accountId: event.target.value })} className="field"><option value="">Pilih akun trading...</option>{accounts.map((account) => <option value={account.id} key={account.id}>{account.name} ({account.currency})</option>)}</select><input required type="number" min="0" step="any" value={snapshotForm.equity} onChange={(event) => setSnapshotForm({ ...snapshotForm, equity: event.target.value })} placeholder="Total equity" className="field" /><input type="datetime-local" value={snapshotForm.recordedAt} onChange={(event) => setSnapshotForm({ ...snapshotForm, recordedAt: event.target.value })} className="field" /><input value={snapshotForm.note} onChange={(event) => setSnapshotForm({ ...snapshotForm, note: event.target.value })} placeholder="Catatan (opsional)" className="field" /><button disabled={saving} className="w-full rounded-xl bg-violet-600 py-2.5 text-sm font-bold text-white disabled:opacity-50">{saving ? "Menyimpan..." : "Simpan snapshot"}</button></form></div>}
